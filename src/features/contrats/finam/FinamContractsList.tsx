// src/features/contrats/finam/FinamContractsList.tsx
...
import { FinamContrat } from '@/types/finam'
...
const getCategorieColor = (categorie?: string) => {
  const colors: Record<string, string> = {
    'personnel finam': 'bg-blue-100 text-blue-800 border-blue-200',
    'retraités': 'bg-amber-100 text-amber-800 border-amber-200',
  }
  const key = (categorie || '').toLowerCase()
  return colors[key] || 'bg-gray-100 text-gray-800 border-gray-200'
}

export const FinamContractsList = () => {
  ...
  const contrats = search.trim() 
    ? allContrats.filter((c: FinamContrat) => 
  ...
  console.log('💰 FINAM - Contrats reçus:', contrats.length, contrats.slice(0, 2))
  ...
  return (
    <div className="space-y-6 p-6 min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50">
      {/* Header */}
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600">
            Micro-assurance Prêts FINAM • {contrats.length} contrat(s) trouvé(s)
          </p>
        </div>
        <Button
          onClick={() => navigate('/contrats/nouveau/finam')}
          className="bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 shadow-lg px-6"
        >
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              >
                ...
                <option value="Personnel FINAM">Personnel FINAM</option>
          ...
          {contrats.length === 0 ? (
            <div className="text-center py-16 px-4">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">
                Aucun contrat FINAM trouvé
              </h3>
              <p className="text-gray-500 mb-8 max-w-md mx-auto">
                Créez votre premier contrat FINAM pour commencer à gérer vos micro-assurances.
              </p>
              <Button
                onClick={() => navigate('/contrats/nouveau/finam')}
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-right text-xs font-bold text-gray-900 uppercase tracking-wider">Montant Assuré</th>
                    <th className="px-6 py-4 text-right text-xs font-bold text-gray-900 uppercase tracking-wider">Prime</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {contrats.map((contrat: FinamContrat) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right font-bold text-emerald-600 text-lg">
                        {formatCurrency(contrat.montant_a_assurer || 0)}
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right space-x-1">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation()
                            navigate(`/contrats/finam/${contrat.id}`)
                          }}
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation()
                            navigate(`/contrats/finam/${contrat.id}/edit`)
                          }}
...
export default FinamContractsList
