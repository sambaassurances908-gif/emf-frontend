...
import { useArianeFinanceContract } from '@/hooks/useArianeFinanceContracts'
import { formatCurrency, formatDate } from '@/lib/utils'
import { ArianeFinanceContratPrint } from './ArianeFinanceContratPrint'
import {
  ArrowLeft,
  User,
  Shield,
...
const DUREE_MAX_MOIS = 60

export const ArianeFinanceContratDetailPage = () => {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const { data: contrat, isLoading, isError } = useArianeFinanceContract(Number(id))
  ...
  useEffect(() => {
    if (isError) {
      ...
      navigate('/contrats/ariane-finance')
    }
  }, [isError, navigate])
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Contrat ARIANE FINANCE #${contrat?.id} - ${getNomComplet()}</title>
          ${styles}
          <style>
  ...
  const getNomComplet = () => {
    if (!contrat) return ''
    if (contrat.nom_prenom) return contrat.nom_prenom
    const parts = [contrat.prenom, contrat.nom].filter(Boolean)
    return parts.length > 0 ? parts.join(' ') : ''
  }

  // Nom complet du bénéficiaire
  const getBeneficiaireNomComplet = () => {
    if (!contrat) return ''
    const parts = [contrat.beneficiaire_prenom, contrat.beneficiaire_nom].filter(Boolean)
  ...
  if (isLoading || !contrat) {
    return (
      <div className="flex items-center justify-center h-[calc(100vh-4rem)] bg-[#FAFAFA]">
        <LoadingSpinner size="lg" text="Chargement du contrat ARIANE FINANCE..." />
      </div>
    )
  }
  ...
  const primeDeces = contrat.prime_totale || Math.round(montant * (TAUX_DECES_IAD / 100))
    ...
    <div className="min-h-screen bg-[#FAFAFA] p-8">
      ...
      <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => navigate(`/sinistres/nouveau/ariane-finance?contrat_id=${contrat.id}&contrat_type=ContratArianeFinance`)}
      ...
      {showContratOfficiel && (
        <div className="bg-white p-6 rounded-3xl shadow-soft border border-gray-100/50 mb-8">
          ...
          <div ref={printRef} className="overflow-auto max-h-[800px] rounded-xl border border-gray-200">
            <ArianeFinanceContratPrint contrat={contrat} />
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-4 bg-white p-6 rounded-3xl shadow-soft border border-gray-100/50 flex flex-col justify-between min-h-[280px]">
          ...
          <div className="space-y-2 pt-4 border-t border-gray-100">
            ...
            <InfoRow label="Durée" value={`${contrat.duree_pret || 0} mois`} />
          </div>
        </div>
        ...
        <InfoCard title="Assuré Principal" icon={User} className="col-span-12 lg:col-span-4">
          <div className="space-y-1">
            <div className="text-xl font-bold text-gray-900 mb-4">{getNomComplet()}</div>
        ...
        <InfoCard title="Garanties" icon={Shield} className="col-span-12 lg:col-span-4">
          <div className="space-y-1">
            <GarantieBadge active={true} label={`Décès / IAD (${TAUX_DECES_IAD}%)`} />
          </div>

          <div className="mt-4 pt-4 border-t border-gray-100">
            <div className="flex justify-between items-center">
              <span className="text-gray-500 text-sm">Prime TTC</span>
              <span className="text-xl font-bold text-samba-green">
                {formatCurrency(primeDeces)}
              </span>
            </div>
          </div>
        </InfoCard>
        ...
        <InfoCard title="Périodes du Contrat" icon={Calendar} className="col-span-12 lg:col-span-6">
          ...
          <div className="mt-4 pt-4 border-t border-gray-100">
            <InfoRow label="Durée totale" value={`${contrat.duree_pret || 0} mois`} />
          </div>
        </InfoCard>
        ...
        <InfoCard title="EMF Souscripteur" icon={Building2} className="col-span-12 lg:col-span-6">
          <div className="space-y-2">
            <div className="text-lg font-bold text-gray-900 mb-2">
              {contrat.emf?.raison_sociale || 'ARIANE FINANCE'}
            </div>
            <InfoRow label="Sigle" value={contrat.emf?.sigle || 'ARIANE FINANCE'} />
          </div>

          <div className="mt-4 bg-gray-50 rounded-xl p-4">
            <div className="text-xs text-gray-500 mb-2">Limites ARIANE FINANCE</div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-600">Montant max:</span>
              <span className="font-bold text-gray-900">{formatCurrency(MONTANT_MAX_PRET)}</span>
            </div>
            <div className="flex justify-between text-sm mt-1">
              <span className="text-gray-600">Durée max:</span>
              <span className="font-bold text-gray-900">{DUREE_MAX_MOIS} mois</span>
            </div>
            <div className="flex justify-between text-sm mt-1">
              <span className="text-gray-600">Taux Décès-IAD:</span>
              <span className="font-bold text-gray-900">{TAUX_DECES_IAD}%</span>
            </div>
          </div>
        </InfoCard>
        ...
        <div className="col-span-12 flex flex-col sm:flex-row justify-end gap-4">
          <Button
            variant="outline"
            onClick={() => navigate(`/contrats/ariane-finance/${contrat.id}/edit`)}
...
export default ArianeFinanceContratDetailPage
