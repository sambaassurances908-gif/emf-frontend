// src/features/contrats/arianefinance/ArianeFinanceContractsList.tsx
...
import { useArianeFinanceContracts } from '@/hooks/useArianeFinanceContracts'
...
export const ArianeFinanceContractsList = () => {
    const navigate = useNavigate()
    const emfId = 9 // ARIANE FINANCE EMF ID fixe
    ...
    const {
        ...
        refetch,
        error
    } = useArianeFinanceContracts(emfId)
    ...
    const contrats = search.trim()
        ? allContrats.filter((c: ArianeFinanceContrat) =>
            `${c.nom} ${c.prenom}`.toLowerCase().includes(search.toLowerCase()) ||
            c.numero_police?.toLowerCase().includes(search.toLowerCase()) ||
            c.telephone?.includes(search) ||
            c.beneficiaire_telephone?.includes(search)
        )
        : allContrats

    console.log('💎 ARIANE FINANCE - Contrats reçus:', contrats.length, contrats.slice(0, 2))
    ...
    return (
        <div className="space-y-6 p-6 min-h-screen bg-gradient-to-br from-violet-50 via-purple-50 to-indigo-50">
            {/* Header */}
                    ...
                    {contrats.length === 0 ? (
                        <div className="text-center py-16 px-4">
                            ...
                            <h3 className="text-xl font-semibold text-gray-900 mb-2">
                                Aucun contrat ARIANE FINANCE trouvé
                            </h3>
                                    ...
                                    {contrats.map((contrat: ArianeFinanceContrat) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <Badge className="px-3 py-1 rounded-full text-xs font-semibold bg-violet-100 text-violet-800 border-violet-200">
                                                    {contrat.ville || 'N/A'}
                                                </Badge>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                                                {contrat.date_effet ? formatDate(contrat.date_effet) : 'N/A'}
                                            </td>
