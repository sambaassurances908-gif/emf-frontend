...
export const EdgContractCreate = () => {
  const navigate = useNavigate()
  const { user } = useAuthStore()
  const emfId = user?.emf_id || 5 // adapte selon ton mapping
  ...
  const { mutate: createContract, isPending } = useCreateEdgContract()
  ...
  const cotisation = formData.montant_pret_assure * 0.025 // ex: 2,5 %
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createContract(payload, {
      onSuccess: (data: any) => {
        navigate(`/contrats/edg/${data.data.id}`)
      },
    })
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        <Button variant="outline" onClick={() => navigate('/contrats/edg')}>
          <ArrowLeft className="h-4 w-4 mr-1" />
          Retour
        </Button>
        <h1 className="text-2xl font-bold">Nouveau contrat EDG</h1>
      </div>
