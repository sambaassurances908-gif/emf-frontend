...
const DateInput: React.FC<{ value: string, onChange: (val: string) => void, readOnly?: boolean }> = ({ value, onChange, readOnly }) => {
    ...
    // Initialize state only on mount or if value changes meaningfully
...
export const BcegTaxiPrevoyanceDecesCreate: React.FC = () => {
    const navigate = useNavigate()
    const { mutate: createContract, isPending } = useCreateBcegTaxiPrevoyanceDecesContract()
    const userEmfId = useAuthStore(state => state.user?.emf_id)

    // State
    const [formData, setFormData] = useState<BcegTaxiPrevoyanceDecesFormData>({
        emf_id: userEmfId || 0,
        numero_police: '',
    ...
    const handleSubmit = async () => {
        ...
        createContract(payload, {
            onSuccess: (data) => {
                navigate(`/contrats/bceg-taxi-prevoyance-deces/${data.id}`, {
            ...
            <div className="bg-white w-[210mm] min-h-[297mm] p-[10mm] shadow-2xl relative flex flex-col border border-gray-300 print:shadow-none print:border-none print:w-full print:p-0">
                ...
                <div className="mt-10 mb-8">
                    <div className="text-right text-[11px] mb-8 pr-12 font-medium">
                        Fait à <input className="border-b border-black w-56 text-center outline-none bg-transparent" placeholder="Libreville" value="Libreville" readOnly />, le <span className="font-bold">{new Date().getDate()}</span> / <span className="font-bold">{new Date().getMonth() + 1}</span> / <span className="font-bold">{new Date().getFullYear()}</span>
                    </div>
