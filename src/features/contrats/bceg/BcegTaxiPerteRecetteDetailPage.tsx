...
import { useBcegTaxiPerteRecetteContract, useDeleteBcegTaxiPerteRecetteContract } from '@/hooks/useBcegTaxiContracts'
import { BcegTaxiPerteRecettePrint } from './BcegTaxiPerteRecettePrint'
import { TAXI_PERTE_RECETTE_CONSTANTS } from '@/types/bcegTaxi'
import {
    ArrowLeft,
    User,
    Edit3,
...
export const BcegTaxiPerteRecetteDetailPage = () => {
    const { id } = useParams<{ id: string }>()
    const navigate = useNavigate()
    const { data: contrat, isLoading, isError } = useBcegTaxiPerteRecetteContract(id || '')
    const { mutate: deleteContract } = useDeleteBcegTaxiPerteRecetteContract()
    ...
    const cotisation = contrat.periodicite === 'semestre'
        ? TAXI_PERTE_RECETTE_CONSTANTS.COTISATION_SEMESTRE
        : TAXI_PERTE_RECETTE_CONSTANTS.COTISATION_ANNUELLE
        ...
        <div className="min-h-screen bg-[#FAFAFA] p-8">
            ...
            <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-10">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            <h1 className="text-2xl font-bold text-gray-900">Contrat Taxi #{contrat.id}</h1>
                            <StatusBadge />
                        </div>
                        <p className="text-sm text-gray-400 mt-1">
                            {nomComplet} • PERTE DE RECETTES
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        onClick={() => navigate(`/sinistres/nouveau/bceg?contrat_id=${contrat.id}&contrat_type=ContratBcegTaxiPerteRecette`)}
            ...
            {showContratOfficiel && (
                <div className="bg-white p-6 rounded-3xl shadow-soft border border-gray-100/50 mb-8">
                    ...
                    <div ref={printRef} className="overflow-auto max-h-[800px] rounded-xl border border-gray-200 bg-gray-100 p-8">
                        <BcegTaxiPerteRecettePrint contrat={contrat} />
                    </div>
                </div>
            )}
            ...
            <div className="grid grid-cols-12 gap-6">
                ...
                <InfoCard title="Garanties" icon={AlertTriangle} className="col-span-12 lg:col-span-4">
                    <div className="space-y-3">
                        <InfoRow label="Indemnité journalière" value={`${formatCurrency(TAXI_PERTE_RECETTE_CONSTANTS.INDEMNITE_JOUR)} FCFA`} />
                        <InfoRow label="Durée max" value={`${TAXI_PERTE_RECETTE_CONSTANTS.DUREE_MAX_JOURS} jours`} />
                        <InfoRow label="Plafond annuel" value={`${formatCurrency(TAXI_PERTE_RECETTE_CONSTANTS.PLAFOND_ANNUEL)} FCFA`} />
                    </div>
                </InfoCard>
            ...
            <div className="mt-10 flex justify-end gap-3 pt-6 border-t border-gray-200">
                <button
                    onClick={() => navigate(`/contrats/bceg-taxi-perte-recette/${id}/edit`)}
