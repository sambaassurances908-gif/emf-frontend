...
import { useBcegRecentContracts } from '@/hooks/useBcegRecentContracts'
...
export const BcegDashboard = () => {
  ...
  // Récupérer emf_id depuis state (LoginPage) ou localStorage ou user ou défaut à 3 (BCEG)
  ...
  } = useBcegRecentContracts(emfId, 5)
  ...
  if (statsLoading || contratsLoading) {
    return (
      <div className="flex items-center justify-center h-[calc(100vh-4rem)]">
        <LoadingSpinner size="lg" text={`Chargement du dashboard BCEG... (EMF #${emfId})`} />
      </div>
    )
  }
  ...
  if (statsError) {
    return (
      <div className="p-6 text-center text-red-600 bg-red-50 rounded-lg mx-6 mt-6">
        ...
        <p>Impossible de charger les statistiques BCEG (EMF #{emfId}).</p>
  ...
  return (
    <div className="space-y-6 p-6 min-h-screen bg-gradient-to-br from-green-50 to-emerald-100">
      {/* Header */}
      <div>
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-3xl font-bold text-green-600 mb-2 flex items-center gap-2">
              🌱 Dashboard BCEG EMF #{emfId}
            </h1>
            <p className="text-gray-600">
              Bonjour {user?.name} - Gestion des contrats micro-assurance BCEG
            </p>
            <p className="text-xs text-green-600 bg-green-50 px-3 py-1 rounded-full mt-1 inline-block">
              EMF ID: {emfId} | {totalContrats} contrats
            </p>
          </div>
          <div className="flex gap-3">
            <Button
              onClick={() => navigate('/sinistres/nouveau/bceg')}
              variant="outline"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card className="border-l-4 border-emerald-500 hover:shadow-lg transition-shadow bg-white">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Contrats Actifs</p>
                <p className="text-3xl font-bold text-emerald-600 mt-2">{contratsActifs}</p>
        ...
        <Card className="border-l-4 border-purple-500 hover:shadow-lg transition-shadow bg-white">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              ...
              {/* ✅ Icône portefeuille + sans couleur de fond */}
              <div className="h-14 w-14 rounded-xl flex items-center justify-center border border-purple-200">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card
          className="cursor-pointer hover:shadow-xl transition-all border-2 border-transparent hover:border-green-500 hover:-translate-y-1 bg-white"
          onClick={() => navigate('/contrats/nouveau/bceg')}
        >
          <CardContent className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Créer un contrat BCEG</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent className="p-6">
          {contrats.length > 0 ? (
            <div className="space-y-4">
              {contrats.map((contrat: BcegContrat) => (
                <div
                  key={contrat.id}
                  className="flex items-center justify-between p-4 border rounded-xl hover:border-green-500 hover:bg-green-50 cursor-pointer transition-all"
