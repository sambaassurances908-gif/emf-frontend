...
import { useArianeFinanceStats } from '@/hooks/useArianeFinanceStats'
import { useArianeFinanceRecentContracts } from '@/hooks/useArianeFinanceRecentContracts'
import { LoadingSpinner } from '@/components/shared/LoadingSpinner'
import logoAriane from '@/assets/logo ariane finance.jpg'
import { formatCurrency } from '@/lib/utils'
import { ArianeFinanceContrat } from '@/types/arianeFinance'

// Fonction pour formater les montants en format compact
...
export const ArianeFinanceDashboard = () => {
    ...
    const emfId =
        emfIdFromState || emfIdFromUser || (emfIdFromStorage ? parseInt(emfIdFromStorage) : 9)
    ...
    const {
        ...
        isError: statsError,
        error: statsErrorObj,
    } = useArianeFinanceStats(emfId)

    const { data: contrats = [], isLoading: contratsLoading } = useArianeFinanceRecentContracts(emfId, 5)
    ...
    if (statsLoading || contratsLoading) {
        return (
            <div className="flex items-center justify-center h-[calc(100vh-4rem)] bg-samba-bg">
                <LoadingSpinner size="lg" text={`Chargement du dashboard ARIANE FINANCE...`} />
            </div>
        )
    }
            ...
            <div className="p-8 bg-samba-bg min-h-screen">
                <div className="max-w-md mx-auto bg-white p-8 rounded-3xl shadow-soft border border-red-100 text-center">
                    ...
                    <p className="text-gray-500 mb-4">
                        Impossible de charger les statistiques ARIANE FINANCE.
                    </p>
                    ...
                    <Button
                        ...
                        className="bg-fuchsia-600 hover:bg-fuchsia-700 text-white font-bold py-3 px-6 rounded-xl"
                    >
                        Réessayer
                    </Button>
                </div>
        ...
        <div className="min-h-screen bg-samba-bg p-8">
            ...
            <header className="flex justify-between items-center mb-10 flex-wrap gap-4">
                <div className="flex items-center gap-4">
                    <img
                        src={logoAriane}
                        alt="Logo ARIANE FINANCE"
                        className="h-14 w-14 rounded-2xl object-cover shadow-soft"
                    />
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900">Dashboard ARIANE FINANCE</h1>
            ...
            <div className="grid grid-cols-12 gap-6">
                ...
                <div className="col-span-12 lg:col-span-4 bg-white p-6 rounded-3xl shadow-soft border border-gray-100/50">
                    ...
                    <div className="inline-flex items-center gap-2 bg-gray-900 text-white px-3 py-1.5 rounded-full text-xs font-medium mb-8">
                        ...
                        <span className="text-gray-400">ARIANE FINANCE #{emfId}</span>
                    </div>
                ...
                <div className="col-span-12 lg:col-span-4">
                    <ActionCard
                        ...
                        subtitle="Créer un contrat ARIANE"
                        icon={Plus}
                ...
                <div className="col-span-12 bg-white p-6 rounded-3xl shadow-soft border border-gray-100/50">
                    ...
                    {contrats.length > 0 ? (
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody className="text-sm font-medium text-gray-700">
                                    {contrats.map((contrat: ArianeFinanceContrat) => (
                                        <tr
                                            key={contrat.id}
                                        ...
                                        >
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 rounded-full bg-samba-blue/10 flex items-center justify-center">
                                                        <span className="text-samba-blue font-bold text-sm">
                                                            {(contrat.nom || contrat.nom_prenom || '?').charAt(0)}
                                                        </span>
                                                    </div>
                                            ...
                                            <td className="py-4 text-center font-bold text-gray-900 border-b border-gray-50">
                                                {formatCurrency(
                                                    (contrat.montant_pret_assure ?? contrat.prime_totale ?? 0) as number
                                                )}
                                            </td>
                        ...
                        <div className="text-center py-12">
                            ...
                            <Button
                                onClick={() => navigate('/contrats/nouveau/arianefinance')}
                                className="mt-4 bg-fuchsia-600 hover:bg-fuchsia-700 text-white font-bold rounded-xl"
                            >
                                <Plus className="h-5 w-5 mr-2" />
                                Créer un contrat
                            </Button>
