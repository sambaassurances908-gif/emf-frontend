// src/features/sinistres/sodec/SodecSinistresList.tsx
...
export const SodecSinistresList = () => {
  const navigate = useNavigate()
  const { user } = useAuthStore()
  const emfId = user?.emf_id || SODEC_EMF_ID

  const [search, setSearch] = useState('')

  const { data: sinistres = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['sodec-sinistres', emfId, search],
    queryFn: async () => {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <LoadingSpinner size="lg" text="Chargement des sinistres SODEC..." />
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 p-6 min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-red-50">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <Button
          ...
          className="bg-gradient-to-r from-rose-600 to-pink-600 hover:from-rose-700 hover:to-pink-700 shadow-lg px-6"
        >
        ...
        <CardContent className="p-0">
          {sinistres.length === 0 ? (
            <div className="text-center py-16 px-4">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Aucun sinistre SODEC</h3>
              <Button onClick={() => navigate('/sinistres/nouveau/sodec')} className="bg-gradient-to-r from-rose-600 to-pink-600">
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {sinistres.map((sinistre: any) => (
                    <tr key={sinistre.id} className="cursor-pointer hover:bg-rose-50" onClick={() => navigate(`/sinistres/sodec/${sinistre.id}`)}>
                      <td className="px-6 py-4 font-mono text-rose-600 font-semibold">{sinistre.numero_sinistre || `SIN-${sinistre.id}`}</td>
                      <td className="px-6 py-4 font-medium text-gray-900">{sinistre.contrat?.nom_prenom || sinistre.contrat?.nom_prenom_assure_principal || 'N/A'}</td>
                      <td className="px-6 py-4"><Badge className="bg-rose-100 text-rose-800">{getTypeLabel(sinistre.type_sinistre)}</Badge></td>
                      <td className="px-6 py-4 text-sm">{formatDate(sinistre.date_survenance)}</td>
                      <td className="px-6 py-4 text-right font-bold text-rose-600">{formatCurrency(sinistre.montant_reclame || 0)}</td>
                      <td className="px-6 py-4"><Badge className={getStatutColor(sinistre.statut)}>{getStatutLabel(sinistre.statut)}</Badge></td>
                      <td className="px-6 py-4 text-right">
                        <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); navigate(`/sinistres/sodec/${sinistre.id}`) }} className="h-8 w-8 p-0">
