...
export const SodecSinistreDeclarationForm = () => {
  ...
  useEffect(() => {
    if (formData.contrat_id && contrats) {
      ...
      if (contrat) {
        setFormData(prev => ({
          ...prev,
          capital_restant_du: contrat.capital_restant_du || contrat.montant_credit || 0,
        }))
      }
    } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 to-purple-100 p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        {/* Header */}
        <div className="bg-gradient-to-r from-rose-500 to-purple-600 text-white rounded-xl p-6 shadow-lg">
          <h1 className="text-3xl font-bold flex items-center gap-3">
            <AlertCircle className="h-8 w-8" />
            Déclaration de Sinistre SODEC
          </h1>
          <p className="mt-2 text-rose-100">
            Remplissez ce formulaire pour déclarer un sinistre sur un contrat SODEC.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-4">
                  ...
                  >
                    ...
                    {contrats?.map((c: any) => (
                      <option key={c.id} value={c.id}>
                        {c.numero_police} - {c.nom_prenom_assure_principal} ({formatCurrency(c.montant_credit || 0)})
                      </option>
                    ))}
                  </Select>
                ...
                {selectedContrat && (
                  <div className="bg-rose-50 p-4 rounded-lg border border-rose-200">
                    <h4 className="font-semibold text-rose-800 mb-2">Informations du contrat</h4>
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div><span className="text-gray-600">Assuré:</span><span className="font-medium ml-2">{selectedContrat.nom_prenom_assure_principal}</span></div>
                      <div><span className="text-gray-600">N° Police:</span><span className="font-medium ml-2">{selectedContrat.numero_police}</span></div>
                      <div><span className="text-gray-600">Montant crédit:</span><span className="font-medium ml-2 text-rose-600">{formatCurrency(selectedContrat.montant_credit || 0)}</span></div>
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="lieu_sinistre" value={formData.lieu_sinistre} onChange={(e) => setFormData({ ...formData, lieu_sinistre: e.target.value })} placeholder="Ex: Libreville, Franceville" />
                </div>
                <div>
          ...
          <Card className="shadow-lg">
            <CardHeader className="bg-fuchsia-50 border-b">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {Object.entries(documentLabels).map(([key, label]) => {
                      ...
                      return (
                        <div key={key} className={`p-4 border rounded-lg ${isRequired ? 'border-rose-300 bg-rose-50' : 'border-gray-200 bg-gray-50'}`}>
          ...
          <div className="flex gap-4 justify-end">
            <Button type="button" variant="outline" onClick={() => navigate('/sinistres/sodec')} disabled={isPending} className="border-gray-400">Annuler</Button>
            <Button type="submit" className="bg-rose-500 hover:bg-rose-600 px-8" disabled={isPending}>
