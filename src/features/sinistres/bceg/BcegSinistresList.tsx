// src/features/sinistres/bceg/BcegSinistresList.tsx
...
export const BcegSinistresList = () => {
  const navigate = useNavigate()
  const { user } = useAuthStore()
  const emfId = user?.emf_id || BCEG_EMF_ID

  const [search, setSearch] = useState('')

  const { data: sinistres = [], isLoading, isError, refetch } = useQuery({
    queryKey: ['bceg-sinistres', emfId, search],
    queryFn: async () => {
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <LoadingSpinner size="lg" text="Chargement des sinistres BCEG..." />
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 p-6 min-h-screen bg-gradient-to-br from-emerald-50 via-green-50 to-teal-50">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <Button
          ...
          className="bg-gradient-to-r from-emerald-600 to-green-600 hover:from-emerald-700 hover:to-green-700 shadow-lg px-6"
        >
        ...
        <CardContent className="p-0">
          {sinistres.length === 0 ? (
            <div className="text-center py-16 px-4">
              ...
              <h3 className="text-xl font-semibold text-gray-900 mb-2">Aucun sinistre BCEG</h3>
              <Button onClick={() => navigate('/sinistres/nouveau/bceg')} className="bg-gradient-to-r from-emerald-600 to-green-600">
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {sinistres.map((sinistre: any) => (
                    <tr key={sinistre.id} className="cursor-pointer hover:bg-emerald-50" onClick={() => navigate(`/sinistres/bceg/${sinistre.id}`)}>
                      <td className="px-6 py-4 font-mono text-emerald-600 font-semibold">{sinistre.numero_sinistre || `SIN-${sinistre.id}`}</td>
                      <td className="px-6 py-4 font-medium text-gray-900">{sinistre.contrat?.nom_prenom || 'N/A'}</td>
                      <td className="px-6 py-4"><Badge className="bg-emerald-100 text-emerald-800">{getTypeLabel(sinistre.type_sinistre)}</Badge></td>
                      <td className="px-6 py-4 text-sm">{formatDate(sinistre.date_survenance)}</td>
                      <td className="px-6 py-4 text-right font-bold text-emerald-600">{formatCurrency(sinistre.montant_reclame || 0)}</td>
                      <td className="px-6 py-4"><Badge className={getStatutColor(sinistre.statut)}>{getStatutLabel(sinistre.statut)}</Badge></td>
                      <td className="px-6 py-4 text-right">
                        <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); navigate(`/sinistres/bceg/${sinistre.id}`) }} className="h-8 w-8 p-0">
