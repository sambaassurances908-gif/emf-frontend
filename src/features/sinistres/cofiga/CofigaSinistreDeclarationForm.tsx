// src/features/sinistres/cofiga/CofigaSinistreDeclarationForm.tsx
...
import { useCofigaContratsForSinistre, useCreateCofigaSinistre } from '@/hooks/useCofigaSinistres'
...
// Types de sinistre COFIGA (Décès et IAD uniquement)
...
export const CofigaSinistreDeclarationForm = () => {
    const navigate = useNavigate()
    const [searchParams] = useSearchParams()
    // const { user } = useAuthStore() - user no longer needed
    const emfId = 7 // Force COFIGA EMF ID

    const urlContratId = searchParams.get('contrat_id') || ''
    const { data: contrats, isLoading: loadingContrats } = useCofigaContratsForSinistre(emfId)
    const { mutate: createSinistre, isPending } = useCreateCofigaSinistre()
    ...
    useEffect(() => {
        if (formData.contrat_id && contrats) {
            ...
            if (contrat) {
                setFormData(prev => ({ ...prev, capital_restant_du: contrat.capital_restant_du || contrat.montant_pret_assure || 0 }))
            }
        } else {
        ...
        <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50">
            <div className="bg-white border-b border-orange-200 sticky top-0 z-10">
                <div className="max-w-5xl mx-auto px-4 py-4">
                    <div className="flex items-center justify-between">
                        <Button variant="ghost" onClick={() => navigate('/sinistres/cofiga')} className="flex items-center gap-2 text-gray-600 hover:text-gray-900">
                            <ArrowLeft className="h-4 w-4" />
                            Retour aux sinistres
                        </Button>
                        <Badge className="bg-orange-100 text-orange-800 px-3 py-1">🏦 COFIGA</Badge>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-5xl mx-auto px-4 py-8">
                <div className="text-center mb-8">
                    ...
                    <h1 className="text-3xl font-bold text-gray-800">Déclaration de Sinistre COFIGA</h1>
                    <p className="text-gray-500 mt-2 max-w-xl mx-auto">Remplissez ce formulaire pour déclarer un sinistre sur un contrat COFIGA.</p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <Card className="overflow-hidden border-0 shadow-xl bg-white">
                        <CardHeader className="bg-gradient-to-r from-orange-500 to-amber-500 text-white py-5">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <CardDescription className="text-orange-100">Sélectionnez le contrat objet du sinistre</CardDescription>
                                </div>
                            </div>
                        </CardHeader>
                        <CardContent className="p-6">
                            <div className="space-y-4">
                                <div>
                                    <Label className="text-gray-700 font-semibold flex items-center gap-2"><FileText className="h-4 w-4 text-orange-500" />Contrat COFIGA *</Label>
                                ...
                                {selectedContrat && (
                                    <div className="mt-4 p-5 bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl border border-orange-200">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                            ...
                                            <div className="bg-white p-3 rounded-xl shadow-sm"><div className="text-xs text-gray-500 mb-1">Montant assuré</div><div className="font-bold text-orange-600">{formatCurrency(selectedContrat.montant_pret_assure || 0)}</div></div>
                    ...
                    <Card className="overflow-hidden border-0 shadow-xl bg-white">
                        <CardHeader className="bg-gradient-to-r from-red-500 to-orange-500 text-white py-5">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <CardDescription className="text-red-100">Sélectionnez le type de sinistre à déclarer</CardDescription>
                                </div>
                            </div>
                        </CardHeader>
                        <CardContent className="p-6">
                                ...
                                {SINISTRE_TYPES.map((type) => {
                                    ...
                                    return (
                                        <div key={type.value} onClick={() => setFormData({ ...formData, type_sinistre: type.value as SinistreType })} className={cn("relative p-4 rounded-2xl border-2 cursor-pointer transition-all duration-200 hover:shadow-lg", isSelected ? `${type.borderColor} ${type.bgColor} shadow-lg scale-105` : "border-gray-200 bg-white hover:border-gray-300")}>
                                            ...
                                            <div className="text-center"><div className={cn("font-semibold text-sm", isSelected ? type.textColor : "text-gray-700")}>{type.label}</div><div className="text-xs text-gray-500 mt-1">{type.description}</div></div>
                                        </div>
                                    )
                                })}
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                                <div>
                                    <Label className="text-gray-700 font-semibold flex items-center gap-2"><Calendar className="h-4 w-4 text-orange-500" />Date du sinistre *</Label>
                                ...
                                <div>
                                    <Label className="text-gray-700 font-semibold flex items-center gap-2"><MapPin className="h-4 w-4 text-orange-500" />Lieu du sinistre</Label>
                                    <Input value={formData.lieu_sinistre} onChange={(e) => setFormData({ ...formData, lieu_sinistre: e.target.value })} placeholder="Ex: Libreville, Owendo" className="mt-2 h-12" />
                                </div>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                                <div>
                                    <Label className="text-gray-700 font-semibold flex items-center gap-2"><DollarSign className="h-4 w-4 text-orange-500" />Capital restant dû (FCFA) *</Label>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-end pt-4">
                        <Button type="button" variant="outline" onClick={() => navigate('/sinistres/cofiga')} disabled={isPending} className="h-12 px-6 border-gray-300"><ArrowLeft className="h-4 w-4 mr-2" />Annuler</Button>
                        <Button type="submit" className="h-12 px-8 bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-700 hover:to-amber-700 shadow-lg" disabled={isPending}>
...
export default CofigaSinistreDeclarationForm
