// src/features/sinistres/cofiga/CofigaSinistresList.tsx
...
const getTypeLabel = (type?: string) => {
    const labels: Record<string, string> = {
        ...
        'perte_activite': 'Perte d\'activité',
        'autre': 'Autre'
    }
...
export const CofigaSinistresList = () => {
    const navigate = useNavigate()
    const { user } = useAuthStore()
    const emfId = user?.emf_id || COFIGA_EMF_ID

    const [search, setSearch] = useState('')

    const { data: sinistres = [], isLoading, isError, refetch } = useQuery({
        queryKey: ['cofiga-sinistres', emfId, search],
        queryFn: async () => {
    ...
    if (isLoading) {
        return (
            <div className="flex items-center justify-center min-h-[60vh]">
                <LoadingSpinner size="lg" text="Chargement des sinistres COFIGA..." />
            </div>
        )
    }
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-600">
                        {sinistres.length} sinistre(s) trouvé(s)
                    </p>
                </div>
                <Button
                    onClick={() => navigate('/sinistres/nouveau/cofiga')}
                    className="bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-700 hover:to-amber-700 shadow-lg px-6"
                >
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {sinistres.map((sinistre: any) => (
                                        <tr
                                            ...
                                            onClick={() => navigate(`/sinistres/cofiga/${sinistre.id}`)}
                                        >
                                            <td className="px-6 py-4 whitespace-nowrap font-mono text-orange-600 font-semibold">
                                                {sinistre.numero_sinistre || `SIN-${sinistre.id}`}
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">
                                                {sinistre.contrat?.nom} {sinistre.contrat?.prenom || 'N/A'}
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right">
                                                <Button
                                                    ...
                                                    onClick={(e) => {
                                                        e.stopPropagation()
                                                        navigate(`/sinistres/cofiga/${sinistre.id}`)
                                                    }}
...
export default CofigaSinistresList
