// src/features/sinistres/edg/EdgSinistreDetailPage.tsx
...
export const EdgSinistreDetailPage = () => {
  ...
  if (isError || !sinistre) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen space-y-4">
        ...
        <Button onClick={() => navigate('/sinistres/edg')}><ArrowLeft className="h-4 w-4 mr-2" /> Retour</Button>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-50 via-purple-50 to-fuchsia-50 p-6">
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <Button variant="ghost" onClick={() => navigate('/sinistres/edg')} className="mb-2"><ArrowLeft className="h-4 w-4 mr-2" /> Retour</Button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="shadow-lg">
            <CardHeader className="bg-gradient-to-r from-violet-50 to-purple-50 border-b">
              <CardTitle className="flex items-center gap-2"><AlertCircle className="h-5 w-5 text-violet-600" /> Informations</CardTitle>
            </CardHeader>
            <CardContent className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><p className="text-sm text-gray-500">N° Sinistre</p><p className="font-bold text-violet-600">{sinistre.numero_sinistre || `SIN-${sinistre.id}`}</p></div>
          ...
          <Card className="shadow-lg">
            ...
            <CardContent className="p-6 space-y-4">
              <div className="bg-violet-50 p-4 rounded-lg">
                ...
                <p className="text-2xl font-bold text-violet-600">{formatCurrency(sinistre.montant_reclame || 0)}</p>
              </div>
          ...
          <Card className="shadow-lg">
            ...
            <CardContent className="p-6 space-y-4">
              ...
              <Button variant="outline" className="w-full" onClick={() => navigate(`/contrats/edg/${sinistre.contrat_id}`)}>
          ...
          <Card className="shadow-lg">
            <CardHeader className="bg-gradient-to-r from-fuchsia-50 to-pink-50 border-b">
              <CardTitle className="flex items-center gap-2"><User className="h-5 w-5 text-fuchsia-600" /> Assuré</CardTitle>
            </CardHeader>
            <CardContent className="p-6 space-y-4">
              <div><p className="text-sm text-gray-500">Nom & Prénom</p><p className="text-lg font-bold">{sinistre.contrat?.nom_prenom || 'N/A'}</p></div>
              <div><p className="text-sm text-gray-500">EMF</p><p className="font-semibold">EDG</p></div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="shadow-lg">
          <CardHeader className="bg-gradient-to-r from-violet-50 to-purple-50 border-b">
            <CardTitle className="flex items-center gap-2"><Calendar className="h-5 w-5 text-violet-600" /> Historique</CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            <div className="space-y-4">
              <div className="flex items-center gap-4"><div className="w-3 h-3 bg-violet-500 rounded-full"></div><div><p className="font-medium">Déclaré</p><p className="text-sm text-gray-500">{formatDate(sinistre.created_at)}</p></div></div>
            </div>
          </CardContent>
        </Card>
      </div>
