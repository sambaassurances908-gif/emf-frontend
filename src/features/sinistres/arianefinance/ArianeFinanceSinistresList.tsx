// src/features/sinistres/arianefinance/ArianeFinanceSinistresList.tsx
...
export const ArianeFinanceSinistresList = () => {
    const navigate = useNavigate()
    const { user } = useAuthStore()
    const emfId = user?.emf_id || ARIANEFINANCE_EMF_ID

    const [search, setSearch] = useState('')

    const { data: sinistres = [], isLoading, isError, refetch } = useQuery({
        queryKey: ['arianefinance-sinistres', emfId, search],
        queryFn: async () => {
    ...
    if (isLoading) {
        return (
            <div className="flex items-center justify-center min-h-[60vh]">
                <LoadingSpinner size="lg" text="Chargement des sinistres ARIANE FINANCE..." />
            </div>
        )
    }
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {sinistres.map((sinistre: any) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right font-bold text-violet-600">
