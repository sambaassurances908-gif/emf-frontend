// src/features/sinistres/arianefinance/ArianeFinanceSinistreDeclarationForm.tsx
...
import { useArianeFinanceContratsForSinistre, useCreateArianeFinanceSinistre } from '@/hooks/useArianeFinanceSinistres'
...
export const ArianeFinanceSinistreDeclarationForm = () => {
    const navigate = useNavigate()
    const [searchParams] = useSearchParams()
    const { user } = useAuthStore()
    const emfId = user?.emf_id || 9

    const urlContratId = searchParams.get('contrat_id') || ''

    const { data: contrats, isLoading: loadingContrats } = useArianeFinanceContratsForSinistre(emfId)
    const { mutate: createSinistre, isPending } = useCreateArianeFinanceSinistre()
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        console.log('📋 Payload ARIANE FINANCE Sinistre:', payload)
    ...
    if (loadingContrats) {
        return (
            <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-violet-50 to-purple-50">
                <LoadingSpinner size="lg" text="Chargement des contrats ARIANE FINANCE..." />
            </div>
        )
    }
            ...
            <div className="bg-white border-b border-slate-200 sticky top-0 z-10">
                <div className="max-w-5xl mx-auto px-4 py-4">
                    <div className="flex items-center justify-between">
                        ...
                        <Badge className="bg-violet-100 text-violet-800 px-3 py-1">
                            💎 ARIANE FINANCE
                        </Badge>
                    </div>
                </div>
            ...
            <div className="max-w-5xl mx-auto px-4 py-8">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-slate-500 mt-2 max-w-xl mx-auto">
                        Remplissez ce formulaire pour déclarer un sinistre sur un contrat ARIANE FINANCE.
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <Card className="overflow-hidden border-0 shadow-xl bg-white">
                        <CardHeader className="bg-gradient-to-r from-violet-500 to-purple-600 text-white py-5">
                                    ...
                                    >
                                        ...
                                        {contrats?.map((c: any) => (
                                            <option key={c.id} value={c.id}>
                                                #{c.id} - {c.nom} {c.prenom} ({c.numero_police || 'Sans numéro'})
                                            </option>
                                        ))}
                                    </Select>
                                    ...
                                    <div className="mt-4 p-5 bg-gradient-to-br from-violet-50 to-purple-50 rounded-2xl border border-violet-200">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                            <div className="bg-white p-3 rounded-xl shadow-sm">
                                                ...
                                                <div className="font-semibold text-slate-800 truncate">{selectedContrat.nom} {selectedContrat.prenom}</div>
                                            </div>
                                            <div className="bg-white p-3 rounded-xl shadow-sm">
                                            ...
                                            <div className="bg-white p-3 rounded-xl shadow-sm">
                                                ...
                                                <div className="font-bold text-violet-600">{formatCurrency(selectedContrat.montant_pret_assure || 0)}</div>
                                            </div>
                                            <div className="bg-white p-3 rounded-xl shadow-sm">
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-end pt-4">
                        ...
                        <Button
                            type="submit"
                            className="h-12 px-8 bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-700 hover:to-purple-700 shadow-lg"
                            disabled={isPending}
                        >
                            {isPending ? (
                                <>
