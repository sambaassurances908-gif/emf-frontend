// src/features/sinistres/agrpro/AgrProSinistresList.tsx
...
export const AgrProSinistresList = () => {
    const navigate = useNavigate()
    const { user } = useAuthStore()
    const emfId = user?.emf_id || AGRPRO_EMF_ID

    const [search, setSearch] = useState('')

    const { data: sinistres = [], isLoading, isError, refetch } = useQuery({
        queryKey: ['agrpro-sinistres', emfId, search],
        queryFn: async () => {
    ...
    if (isLoading) {
        return (
            <div className="flex items-center justify-center min-h-[60vh]">
                <LoadingSpinner size="lg" text="Chargement des sinistres AGR PRO..." />
            </div>
        )
    }
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {sinistres.map((sinistre: any) => (
                                        <tr
                                            ...
                                            onClick={() => navigate(`/sinistres/agrpro/${sinistre.id}`)}
                                        >
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap font-medium text-gray-900">
                                                {sinistre.contrat?.nom} {sinistre.contrat?.prenom}
                                            </td>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right">
                                                <Button
                                                    ...
                                                    onClick={(e) => {
                                                        e.stopPropagation()
                                                        navigate(`/sinistres/agrpro/${sinistre.id}`)
                                                    }}
