...
export const BambooSinistresList = () => {
  const navigate = useNavigate()
  const { user } = useAuthStore()
  const emfId = user?.emf_id || BAMBOO_EMF_ID
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <LoadingSpinner size="lg" text="Chargement des sinistres BAMBOO..." />
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 p-6 min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
      {/* Header */}
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {sinistres.map((sinistre: any) => (
                    <tr
                      ...
                      onClick={() => navigate(`/sinistres/bamboo/${sinistre.id}`)}
                    >
                      <td className="px-6 py-4 whitespace-nowrap font-mono text-blue-600 font-semibold">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation()
                            navigate(`/sinistres/bamboo/${sinistre.id}`)
                          }}
