// src/features/sinistres/bamboo/BambooSinistreDeclarationForm.tsx
...
import { useBambooContratsForSinistre, useCreateBambooSinistre } from '@/hooks/useBambooSinistres'
...
// Types de sinistre avec icônes et couleurs
const SINISTRE_TYPES = [
  { 
    value: 'deces', 
    label: 'Décès', 
    description: 'Décès de l\'assuré',
    icon: Heart,
  ...
  { 
    value: 'iad', 
    label: 'Invalidité (IAD)', 
    description: 'Invalidité Absolue et Définitive',
    icon: Shield,
  ...
  { 
    ...
    description: 'Licenciement involontaire',
    icon: Briefcase,
  ...
  { 
    value: 'perte_activite', 
    label: 'Perte d\'activité', 
    description: 'Cessation d\'activité commerciale',
    icon: Store,
...
export const BambooSinistreDeclarationForm = () => {
  const navigate = useNavigate()
  const [searchParams] = useSearchParams()
  const { user } = useAuthStore()
  const emfId = user?.emf_id || 1

  // Récupérer le contrat_id depuis l'URL si présent
  const urlContratId = searchParams.get('contrat_id') || ''

  const { data: contrats, isLoading: loadingContrats } = useBambooContratsForSinistre(emfId)
  const { mutate: createSinistre, isPending } = useCreateBambooSinistre()
  ...
  // Pré-sélectionner le contrat depuis l'URL si disponible
  useEffect(() => {
  ...
  useEffect(() => {
    if (formData.contrat_id && contrats) {
      ...
      if (contrat) {
        setFormData(prev => ({
          ...prev,
          capital_restant_du: contrat.capital_restant_du || contrat.montant_pret_assure || 0,
        }))
      }
    } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload: SinistreCreatePayload = {
      ...
      date_sinistre: formData.date_sinistre, // Format YYYY-MM-DD depuis input type="date"
      capital_restant_du: capitalRestantDu,
    }

    // Champs optionnels - ne pas envoyer si vide
    if (formData.circonstances && formData.circonstances.trim()) {
      payload.circonstances = formData.circonstances.trim()
    }
    if (formData.lieu_sinistre && formData.lieu_sinistre.trim()) {
      payload.lieu_sinistre = formData.lieu_sinistre.trim()
    }
    if (formData.montant_reclame && Number(formData.montant_reclame) > 0) {
      payload.montant_reclame = Number(formData.montant_reclame)
    }

    // Documents PDF
    Object.entries(documents).forEach(([key, file]) => {
      if (file instanceof File) {
        (payload as any)[key] = file
      }
    })
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 mt-2 max-w-xl mx-auto">
            Remplissez ce formulaire pour déclarer un sinistre sur un contrat d'assurance emprunteur BAMBOO.
          </p>
        </div>
            ...
            {steps.map((step, index) => (
              <div key={step.num} className="flex items-center">
                ...
                {index < steps.length - 1 && (
                  <div className={cn(
                    "w-8 md:w-16 h-1 mx-1 rounded-full",
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <Card className="overflow-hidden border-0 shadow-xl bg-white">
            <CardHeader className="bg-gradient-to-r from-blue-500 to-blue-600 text-white py-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription className="text-blue-100">Sélectionnez le contrat objet du sinistre</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-6">
              <div className="space-y-4">
                <div>
                  <Label htmlFor="contrat" className="text-slate-700 font-semibold flex items-center gap-2">
                  ...
                  <Select
                    ...
                    className={cn(
                      "mt-2 h-12 text-base",
                      errors.contrat_id ? 'border-red-500 ring-red-200' : 'border-slate-300'
                    )}
                  >
                    <option value="">-- Sélectionnez un contrat --</option>
                    {contrats?.map((c: any) => (
                      <option key={c.id} value={c.id}>
                        #{c.id} - {c.nom_prenom} ({c.numero_police || 'Sans numéro'})
                      </option>
                    ))}
                  </Select>
                ...
                {selectedContrat && (
                  <div className="mt-4 p-5 bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl border border-blue-200">
                    <div className="flex items-center gap-2 mb-4">
                      <CheckCircle className="h-5 w-5 text-green-500" />
                      <span className="font-semibold text-slate-800">Contrat sélectionné</span>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      <div className="bg-white p-3 rounded-xl shadow-sm">
                        <div className="text-xs text-slate-500 mb-1">Assuré</div>
                        <div className="font-semibold text-slate-800 truncate">{selectedContrat.nom_prenom}</div>
                      </div>
                      <div className="bg-white p-3 rounded-xl shadow-sm">
                        <div className="text-xs text-slate-500 mb-1">N° Police</div>
                        <div className="font-mono font-semibold text-slate-800">{selectedContrat.numero_police || 'N/A'}</div>
                      </div>
                      <div className="bg-white p-3 rounded-xl shadow-sm">
                        <div className="text-xs text-slate-500 mb-1">Montant assuré</div>
                        <div className="font-bold text-blue-600">{formatCurrency(selectedContrat.montant_pret_assure || 0)}</div>
                      </div>
                      <div className="bg-white p-3 rounded-xl shadow-sm">
                        <div className="text-xs text-slate-500 mb-1">Statut</div>
                        <Badge className={cn(
                          "font-medium",
                          selectedContrat.statut === 'actif' ? 'bg-green-100 text-green-800' : 'bg-amber-100 text-amber-800'
                        )}>
          ...
          <Card className="overflow-hidden border-0 shadow-xl bg-white">
            ...
            <CardContent className="p-6">
              {/* Cards pour les types de sinistre */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                {SINISTRE_TYPES.map((type) => {
                  ...
                  return (
                    <div
                      key={type.value}
                      onClick={() => setFormData({ ...formData, type_sinistre: type.value as SinistreType })}
                      className={cn(
                        "relative p-4 rounded-2xl border-2 cursor-pointer transition-all duration-200 hover:shadow-lg",
                        isSelected 
                    ...
                    >
                      ...
                      <div className={cn(
                        ...
                        isSelected ? `bg-gradient-to-br ${type.color}` : "bg-slate-100"
                      )}>
                      ...
                      <div className="text-center">
                        <div className={cn(
                          ...
                          isSelected ? type.textColor : "text-slate-700"
                        )}>
                          {type.label}
                        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                  ...
                  <Input
                    ...
                    className={cn("mt-2 h-12", errors.date_sinistre ? 'border-red-500' : '')}
                  />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                <div>
                  <Label className="text-slate-700 font-semibold flex items-center gap-2">
                    ...
                    Capital restant dû (GNF) *
                  </Label>
                  <Input
                    type="number"
                    value={formData.capital_restant_du}
                    onChange={(e) => setFormData({ ...formData, capital_restant_du: parseFloat(e.target.value) || 0 })}
                    className={cn("mt-2 h-12", errors.capital_restant_du ? 'border-red-500' : '')}
                  />
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData({ ...formData, montant_reclame: parseFloat(e.target.value) || 0 })}
                    placeholder="0"
                    className="mt-2 h-12"
                  />
                </div>
          ...
          {/* CARD 3: Documents justificatifs */}
          <Card className="overflow-hidden border-0 shadow-xl bg-white">
                  ...
                  <div className="mb-6 p-4 bg-indigo-50 rounded-xl border border-indigo-200 flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-indigo-600 mt-1">
                        Les documents marqués d'un <span className="text-red-500 font-bold">*</span> sont obligatoires.
                      </p>
                    </div>
                  </div>
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            <Label className="font-medium text-slate-700 flex items-center gap-2">
                              <FileCheck className={cn("h-4 w-4", file ? "text-green-500" : isRequired ? "text-purple-500" : "text-slate-400")} />
                              {label}
                            ...
                            {file && (
                              <Button
                                ...
                                className="text-red-500 hover:bg-red-100 h-7 w-7 p-0"
                              >
                                <X className="h-4 w-4" />
                              </Button>
                            )}
